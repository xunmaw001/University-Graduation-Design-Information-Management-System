	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="upload" v-if="type!='info'&& !ro.lunwen" label="论文" prop="lunwen">
					<file-upload
						tip="点击上传论文"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.lunwen?ruleForm.lunwen:''"
						@change="lunwenUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.lunwen" label="论文" prop="lunwen">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"rgba(198, 205, 214, 1)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.lunwen)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="!ruleForm.lunwen" label="论文" prop="lunwen">
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="提交时间" prop="tijiaoshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.tijiaoshijian" label="提交时间" prop="tijiaoshijian">
					<el-input v-model="ruleForm.tijiaoshijian" placeholder="提交时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-if="type!='info'"  label="开题报告" prop="kaitibaogao">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.kaitibaogao" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.kaitibaogao" label="开题报告" prop="kaitibaogao">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.kaitibaogao"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='lunwen'){
							this.ruleForm.lunwen = obj[o];
							this.ro.lunwen = true;
							continue;
						}
						if(o=='kaitibaogao'){
							this.ruleForm.kaitibaogao = obj[o];
							this.ro.kaitibaogao = true;
							continue;
						}
						if(o=='daoshigonghao'){
	...
	this.ruleForm.kaitibaogao = this.ruleForm.kaitibaogao.replace(reg,'../../../ssm22t68/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.lunwen!=null) {
		this.ruleForm.lunwen = this.ruleForm.lunwen.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `lunwentijiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.lunwentijiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    lunwenUploadChange(fileUrls) {
	    this.ruleForm.lunwen = fileUrls;
    },
  }
};
</script>
