...
<div :style='{"width":"calc(100% - 40px)","padding":"30px 20px 40px","margin":"20px auto 0","position":"relative","background":"rgba(255,255,255,.3)"}'>
    ...
    >
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="答辩ppt" prop="dabianppt">
            <file-upload
            tip="点击上传答辩ppt"
            action="file/upload"
            :limit="1"
            :multiple="true"
            :fileUrls="ruleForm.dabianppt?ruleForm.dabianppt:''"
            @change="dabianpptUploadChange"
            ></file-upload>
          </el-form-item>  
          ...
          <el-form-item :style='{"border":"0px solid #dfdfdf","padding":"10px","boxShadow":"0px 0px 0px #eee","margin":"0 0 8px 0","borderRadius":"8px","background":"none"}' label="答辩时间" prop="dabianshijian" >
              <el-date-picker
                  format="yyyy 年 MM 月 dd 日"
                  value-format="yyyy-MM-dd"
                  v-model="ruleForm.dabianshijian" 
                  type="date"
                  placeholder="答辩时间">
              </el-date-picker> 
          </el-form-item>
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.dabianshijian = this.getCurDate()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='dabianshijian'){
              this.ruleForm.dabianshijian = obj[o];
              continue;
            }
          }
        }
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('lunwendabian/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('lunwendabian/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('lunwendabian/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      dabianpptUploadChange(fileUrls) {
          this.ruleForm.dabianppt = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
